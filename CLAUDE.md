...
- **Overlay position anchored to max height** — `_position_window()` computes `y` using the maximum possible overlay height (`_ENTRY_AREA_H + _STRIP_HEIGHT + _MAX_ROWS_VISIBLE * _ROW_HEIGHT + _COUNT_BAR_H`) rather than the current height. This keeps the search box at a fixed screen position while the list grows or shrinks below it.
- **Result count footer** — a fixed `_COUNT_BAR_H` (18 px) `tk.Frame` sits below the canvas and shows a right-aligned `"N result(s)"` label, updated on every `_refresh_canvas` call. It replaces the former vertical scrollbar; the overlay is keyboard-only (no mouse click activation). `_COUNT_BAR_H` is included in both `overlay_h` and `max_overlay_h` in `_position_window` so the anchor stays stable. `_count_label` is set to `None` in `hide()` like other widget references.
