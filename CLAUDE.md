...
Windows Navigator is a keyboard-driven window switcher for Windows. Press **Win+Alt+Space** to open an overlay listing all open windows; type to filter, arrow keys to navigate, Enter to focus, **Ctrl+Enter** to move the window to the current virtual desktop and focus it.
...
src/windows_navigator/
    ...
    filter.py            # filter_windows(), parse_query() — pure Python, no Win32
...
tests/
    test_filter.py           # filter_windows(), parse_query()
...
| `filter.py` | Pure Python: `parse_query` splits a query into `(desktop_nums, text)`; `filter_windows` applies it to a window list |
| `models.py` | `WindowInfo` and `TabInfo` dataclasses |
...
3. Current desktop number is derived from windows (first `is_current_desktop=True` entry) and passed as `initial_query="#N"` to `overlay.show()`
4. Overlay resets with fresh windows, renders `#N` as a coloured badge widget in the search entry (text field starts empty), shows via Tk `Toplevel`
...
- **Desktop number search prefix** — `filter.py` loops over consecutive leading `#N` tokens (e.g. `#2#3 chrome`) and restricts results to windows on any of those desktops (OR logic) before applying the text filter. The overlay opens with the current desktop number rendered as a coloured badge. Multiple badges are supported. Backspace on an empty entry removes the rightmost badge (bell badge if active, otherwise the rightmost desktop badge). When the last badge is gone, the query is empty and all windows are shown. `#` alone (no digit) is treated as an empty query.
...
- **Dynamic overlay resize** — `_on_text_changed` calls `_resize_to_fit()` after every keystroke, which updates the canvas height and calls `_position_window()`. This ensures the window expands/shrinks as results change (e.g. clearing the `#3` badge to show all windows). Maximum visible rows: 10.
...
- **Overlay position anchored to max height** — `_position_window()` computes `y` using the maximum possible overlay height (`56 + _STRIP_HEIGHT + _MAX_ROWS_VISIBLE * _ROW_HEIGHT`) rather than the current height. This keeps the search box at a fixed screen position while the list grows or shrinks below it.
...
- **Desktop prefix badge in the search entry** — the entry area is a composite `entry_inner` frame with `tk.Label` prefix badges on the left and a `tk.Entry` on the right. Desktop prefix badges are added only via Ctrl+1–9 (not by typing `#N`) and stored in `_desktop_prefix_nums`; `_effective_query()` prepends them as `#N` tokens to the entry text before passing to the controller. Backspace at caret position 0 removes the rightmost badge (bell first, then desktop prefix badges right-to-left). **`<KeyRelease>` fires for arrows too**: `_on_text_changed` only calls `set_query` when the effective query has actually changed — without this guard any arrow key resets `selection_index` to 0.
...
- **`show()` `fg_hwnd` parameter** — `show(windows, initial_query="", fg_hwnd=0)` stores the foreground HWND captured at hotkey time in `self._show_fg_hwnd`. `_grab_focus` uses it as a fallback `attach_to` when `GetForegroundWindow()` at call time returns 0 or the overlay's own HWND (see Overlay focus note above).
...
| File | What it covers |
|------|----------------|
| `test_filter.py` | `filter_windows` — text match, multi-token non-contiguous match, single and multi `#N` prefix (OR logic), edge cases (`#` alone returns all, non-digit, multi-digit, trailing space); `parse_query` — direct unit tests for all normalisation cases (bare `#`, whitespace-only, trailing space, `#abc`, multi-digit, chained prefixes) |
| `test_keyboard.py` | `OverlayController` — navigation (arrow, page, boundary), query, reset, `all_windows` invariant, desktop prefix; `app_icons`, `text_filtered_windows`, `cycle_app_filter`, `clear_app_filter`, auto-clear on query change; tab search (`_tab_query_matches` gating, matching-only tab display, desktop filter, multi-token, app-filter preservation); `set_tabs`, `tab_count`, `is_expanded`; `toggle_expansion` (expand/collapse, single-tab guard, flat_list shape, selection lands on parent); `toggle_all_expansions` (expand-all, collapse-when-any-expanded, single-tab skip, filter-aware, non-visible windows untouched, deferred `_want_all_expanded` flag set before tabs arrive and auto-expand on `set_tabs`) |
| `test_provider.py` | `IconExtractor.extract` fallback on non-Windows |
| `test_virtual_desktop.py` | `is_on_current_desktop`, `assign_desktop_numbers` (registry order, None GUID, per-window exception), `get_current_desktop_number` (mocked winreg) |
