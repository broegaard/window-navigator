...
Windows Navigator is a keyboard-driven window switcher for Windows. Press **Ctrl+Shift+Space** to open an overlay listing all open windows; type to filter, arrow keys to navigate, Enter to focus, **Ctrl+Enter** to move the window to the current virtual desktop and focus it.
...
| `controller.py` | Pure Python state machine — text query, desktop badge filter, app filter, bell filter, tab state (`_tabs`, `_expanded`), `flat_list`, selection index. No Tk dependency. Defines three focused sub-protocols (`FilterControllerProtocol`, `NavigationControllerProtocol`, `TabControllerProtocol`) composed into `OverlayControllerProtocol`. |
...
- **Queue-based hotkey marshaling** — the hotkey thread must not touch Tk directly; it enqueues a token (the foreground HWND at hotkey time, now discarded by `poll_queue`) so the main thread knows a hotkey fired. The main thread dequeues via `poll_queue`.
- **RegisterHotKey instead of keyboard library** — `_start_hotkey_listener` in `app.py` uses Win32 `RegisterHotKey(hwnd=NULL, ...)` on a daemon thread. `WM_HOTKEY` is explicitly exempted by Windows from the foreground-lock timeout, so a plain `SetForegroundWindow` in `_grab_focus` works reliably after alt-tab — no `AttachThreadInput` needed. `AttachThreadInput` is intentionally absent: merging another thread's input queue into ours causes cursor-state corruption (Windows sets `IDC_APPSTARTING` on the shared queue during `deiconify()`, which bleeds into the detached thread after `AttachThreadInput(False)` and produces a persistent spinning cursor in Firefox and Windows Terminal until the mouse moves). A `WH_KEYBOARD_LL` hook (the old `keyboard` library approach) does NOT receive the WM_HOTKEY exemption.
...
- **Activation before hide** — `_activate_selected` and `_move_and_activate_selected` call `_on_activate` / `_on_move` BEFORE `hide()`. This gives the target window one clean `WM_ACTIVATE` rather than two (Windows would auto-activate the previous foreground when the overlay is destroyed, then `activate_window` would activate it again). `_closing = True` is set before the activation call so that `_on_focus_out` — which fires when the overlay loses focus during the handoff — does not schedule a spurious `hide()`. `hide()` resets `_closing` to `False` on entry.
...
- **Overlay focus after click-away / alt+tab** — `_grab_focus` calls `_force_foreground` (defined in `activation.py`) which calls `SetForegroundWindow` directly; the foreground-lock exemption from `WM_HOTKEY` makes this sufficient (see RegisterHotKey note). After `SetForegroundWindow`, `_grab_focus` calls Win32 `SetFocus(entry_hwnd)` directly to route keyboard input to the entry widget, then `focus_force()` to update Tk's internal focus state. **SW_SHOWNORMAL flash**: Tk's `deiconify()` calls `ShowWindow(SW_SHOWNORMAL)` which briefly activates the window; Windows immediately revokes activation → `WM_ACTIVATE(WA_INACTIVE)` → Tk `<FocusOut>` → `_pending_hide` is scheduled. `_on_focus_in` cancels it when `_grab_focus` subsequently re-grabs focus — without this the overlay flashes (appears then immediately closes). **Cursor nudge on hide**: after `destroy()`, Windows does not send `WM_SETCURSOR` to the window beneath until the mouse moves; `hide()` calls `SetCursorPos(x+1, y)` then `SetCursorPos(x, y)` (a 1-px nudge-and-return, invisible to the user) to inject two `WM_MOUSEMOVE` messages that force `WM_SETCURSOR` on the underlying window and prevent a spinning cursor in the former overlay area.
...
- **`_set_query_state` is the only correct way to change badge state** — any handler that modifies `_desktop_prefix_nums` (Ctrl+1–9, Backspace badge removal, Ctrl+/-) must call `_set_query_state(nums, text)`, NOT `_update_prefix_badges(nums)` + `_on_text_changed()`. `_update_prefix_badges` only updates the visual widgets; `_on_text_changed` only calls `controller.set_query` with the entry text. Neither path reaches `controller.set_desktop_nums`, so the controller's desktop filter falls out of sync with the displayed badges. `_set_query_state` is the single call that keeps badges, entry text, and controller state consistent.
...
- **App icon strip** — one 32×32 icon per unique `process_name` in z-order (from `controller.app_icons`). Tab/Shift-Tab cycle the app filter; bound on `self._entry` specifically, not `top`, so they only fire when the entry has keyboard focus (`<ISO_Left_Tab>` also bound for X11). Both `set_query` and `set_desktop_nums` auto-clear `_app_filter` when the active app disappears from the filtered list. Strip photo images live in `self._strip_photo_images` (separate from `self._photo_images` for the list canvas); both are cleared by `hide()`.
...
- **`IVirtualDesktopManagerInternal` — feature detection, not build-number branching** — `probeVDInternal()` in `virtual_desktop_windows.go` calls `IServiceProvider::QueryService` with each known IID from newest to oldest (`_iidVDMI_26100` → `_iidVDMI_9000`) and stops at the first success. The one ambiguous case: build 22449 shares the same IID as 21313 but `GetAllCurrentDesktops` was inserted at vtable[7], shifting `GetDesktops` to index 8 and `SwitchDesktop` to 10. Resolve by reading `CurrentBuildNumber` from `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion` only when the 21313 IID is selected. Older builds (20231–22449) also pass `HWND=0` as a leading argument to `GetCurrentDesktop`, `GetDesktops`, and `SwitchDesktop`.
- **COM STA worker goroutine** — `IVirtualDesktopManagerInternal` uses `CLSCTX_LOCAL_SERVER` (out-of-process, communicates with the shell via a proxy). All calls are serialised through a dedicated goroutine that calls `runtime.LockOSThread()` + `CoInitializeEx(APARTMENTTHREADED)` once at startup, then processes requests from a channel. This ensures STA thread affinity for the lifetime of the process. The ImmersiveShell `IServiceProvider` and the resolved `IApplicationViewCollection` / `IVirtualDesktopManagerInternal` pointers are kept alive for the process lifetime — never Released.
- **DPI manifest `.syso`** — `go/rsrc.syso` is a pre-compiled COFF resource object (generated by `github.com/akavel/rsrc`) that embeds `go/app.manifest` as an `RT_MANIFEST` resource. Go links `.syso` files automatically. The manifest declares `PerMonitorV2` DPI awareness before any DLL or COM initialisation, complementing the programmatic `SetProcessDpiAwareness(2)` call in `app_windows.go`. Regenerate with `go generate ./...` from `go/` after editing `app.manifest`.
