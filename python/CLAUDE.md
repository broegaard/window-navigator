...
- **`_polling_double_tap_listener` handles both polling choices via `tap_vk` and optional `guard_vk` params** — double-tap Ctrl passes `tap_vk_l=VK_LCONTROL, tap_vk_r=VK_RCONTROL` with no guard; hold-Ctrl double-tap-Shift passes `tap_vk_l=VK_LSHIFT, tap_vk_r=VK_RSHIFT, guard_vk_l=VK_LCONTROL, guard_vk_r=VK_RCONTROL`. Hotkey IDs are module-level constants (`_HOTKEY_ID_DOUBLE_TAP_CTRL = 100`, `_HOTKEY_ID_DOUBLE_TAP_SHIFT = 400`, etc.) and must not collide with each other or with the move-hotkey IDs (1 = left, 2 = right) registered in `_start_move_hotkey_listener`. `tap_was_down` is updated every poll cycle regardless of guard state, so the rising-edge latch is correct even when the guard key changes mid-poll. The 300 ms window does not require the guard to be held continuously — a release-and-repress between taps will still trigger if within the window.
...
- **`load_hotkey` silences only `FileNotFoundError`** — a missing config file is the normal first-run state and is caught silently. Any other exception (malformed TOML, invalid enum value, permission error) is logged as `WARNING` via `logging.getLogger(__name__)` with the file path and full traceback so misconfiguration is diagnosable in the application log.
...
- **`move_window_to_adjacent_desktop` aborts on move failure** — if `AppView(hwnd).move()` raises any exception (including `ImportError` when `pyvda` is unavailable), the function returns `0` immediately without calling `switch_to_desktop_number`. Switching the desktop when the window failed to follow would be confusing.
...
- **`_refresh_selection_only` avoids `delete("all")` on navigation key presses** — Arrow, PgUp/PgDn, Ctrl+Home/End handlers capture `old = controller.selection_index` before the move, then call `_refresh_selection_only(old, new)` instead of `_refresh_canvas()`. `_refresh_canvas` tags each row's background rectangle with `f"row_bg_{i}"`; `_refresh_selection_only` updates only those two items via `itemconfig` and recomputes scroll position. All other events that change list content (text filter, app filter, tab toggle) still trigger a full redraw.
