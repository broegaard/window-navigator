...
- **OUTLOOK.EXE is excluded from UIA tab fetching** — Outlook's UIA tree is unusually large and slow to traverse; walking it during `_fetch_tabs_bg` causes a noticeable hang every time the overlay opens. Outlook has no browser-style tab strip, so the fetch produces nothing useful anyway. The skip is a process-name guard (`w.process_name.upper() == "OUTLOOK.EXE"`) at the top of the per-window loop in `_fetch_tabs_bg`.
